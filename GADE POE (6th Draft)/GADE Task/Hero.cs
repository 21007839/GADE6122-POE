    ...
    public class Hero : Character
    {
        ...
        public override void Attack(Character target)
        {
            ...
            if (CheckRange(target))
            {
                ...
                // Check if the user can afford the weapons
                for (int i = 0; i < 3; i++)
                {
                    if (Game.ge.GetGameShop.CanBuy(Game.ge.GetWeaponCost(Game.ge.GetGameShop.GetWeapons[i])))
                    {
                        Game.ge.GetAvailability[i] = true;
                    }
                    else
                    {
                        Game.ge.GetAvailability[i] = false;
                    }
                }
            }
            else
